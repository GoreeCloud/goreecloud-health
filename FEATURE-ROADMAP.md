...
- **Implemented source contracts:** `activity.steps`, `activity.distance`, `activity.active-energy`, `activity.active-time`, `activity.intensity`, `exercise.session`, `sleep.session`, `heart.rate`, `body.weight`, and `hydration.water`, including canonical source-level units where applicable. Activity active energy/time/intensity have bounded source semantics; `sleep.session` optionally carries bounded source-supported stage intervals while stage-less sessions remain valid; `exercise.session` optionally preserves a namespaced source classification while unclassified sessions remain valid.
- **Implemented exercise classification preservation boundary:** a session may retain the source's classification system/code/label without claiming a GoreeCloud canonical activity type or cross-provider equivalence. A future canonical exercise taxonomy/mapping remains separately governed.
...
- Define a governed cross-provider exercise taxonomy/mapping only when provider equivalence can be justified without losing source semantics; add further exercise details such as laps/segments/routes only through separate bounded contracts.
- Expand type-specific normalized contracts for additional vitals, additional body measurements, broader nutrition, and supported wellbeing records.
- Define governed user-facing active-time totals across records/sources, intensity-minute aggregation, and sleep-stage duration/quality analytics only after concrete product use cases are approved; source contracts do not themselves authorize derived totals, scoring, or cross-source aggregation.
