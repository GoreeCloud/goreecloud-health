...
- **Implemented source contracts:** `activity.steps`, `activity.distance`, `activity.active-energy`, `activity.intensity`, `exercise.session`, `sleep.session`, `heart.rate`, `body.weight`, and `hydration.water`, including canonical source-level units where applicable. `activity.active-energy` is a bounded positive-duration interval measurement in `kcal` that excludes basal energy; `activity.intensity` is a positive-duration interval classified only as `moderate` or `vigorous`; the current `exercise.session` payload remains deliberately empty pending governed classification/detail semantics.
...
- Define governed active-time and intensity-minute aggregation semantics only after a concrete product use case is approved; the new `activity.intensity` source contract does not itself authorize duration totals, weighted intensity minutes, or cross-source aggregation.
