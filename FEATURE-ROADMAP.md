...
- Expand type-specific normalized contracts for exercise classification/details beyond the bounded session contract, sleep stages, additional vitals, additional body measurements, broader nutrition, and supported wellbeing records.
- Define governed user-facing active-time totals across records/sources and intensity-minute aggregation semantics only after a concrete product use case is approved; the source contracts do not themselves authorize cross-record totals, weighted intensity minutes, or cross-source aggregation.
