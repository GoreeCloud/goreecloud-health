...
- **Implemented source contracts:** `activity.steps`, `activity.distance`, `activity.active-energy`, `activity.active-time`, `activity.intensity`, `exercise.session`, `sleep.session`, `heart.rate`, `body.weight`, and `hydration.water`, including canonical source-level units where applicable. `activity.active-energy` is a bounded positive-duration interval measurement in `kcal` that excludes basal energy; `activity.active-time` is a source-provided duration in seconds that cannot exceed its enclosing positive observation interval; `activity.intensity` is a positive-duration interval classified only as `moderate` or `vigorous`; `sleep.session` now optionally carries bounded source-supported stage intervals while stage-less sessions remain valid; the current `exercise.session` payload remains deliberately empty pending governed classification/detail semantics.
...
- Expand type-specific normalized contracts for exercise classification/details beyond the bounded session contract, additional vitals, additional body measurements, broader nutrition, and supported wellbeing records.
