...
All current fixtures are repository-owned synthetic records. No Health Connect or real user record is read by this implementation. The active-energy and activity-intensity contracts are normalization boundaries only; they do not calculate energy, authorize ingestion, establish a runtime provider mapping, aggregate duration, or calculate weighted intensity minutes. Exercise classification/details beyond the session boundary, sleep-stage, additional vital/body, broader nutrition, and wellbeing contracts remain planned rather than inferred from these types.

### Android interoperability reference

For future Android interoperability, the `activity.intensity` contract deliberately matches the bounded source semantics of Health Connect's feature-gated `ActivityIntensityRecord`: a start/end interval plus either moderate or vigorous intensity. Health Connect separately exposes aggregate duration and intensity-minute metrics, but GoreeCloud Health does not currently implement or authorize those aggregate behaviors. A future provider adapter must feature-check the platform capability, request only governed permissions, preserve provider provenance, and pass Privacy Shield operation-level authorization/runtime acceptance before real records are processed.

## Reconciliation policy

The Development source contract includes `contracts/health-reconciliation-policy.v1.json` with schema version `goreecloud.health.reconciliation-policy.v1`. It applies exactly to the nine current record types and turns the current conservative reconciliation baseline into an explicit machine-readable policy.
...
4. The current reconciliation policy does not authorize cross-source aggregation or conflict resolution, including for active-energy or activity-intensity records.
