...
- Versioned source-level `goreecloud.health.record.v1` envelope and source contract plus ten current type contracts: `activity.steps`, `activity.distance`, `activity.active-energy`, `activity.active-time`, `activity.intensity`, `exercise.session`, `sleep.session`, `heart.rate`, `body.weight`, and `hydration.water`; all current fixtures are synthetic and fail-closed validation is repository-owned.
...
- `activity.active-time` represents a source-provided active duration in seconds inside a positive observation interval. The value must not exceed that enclosing interval; the contract does not define how a source determines activity and does not derive active time from steps, energy, intensity, exercise, or other records.
- `activity.intensity` represents a positive-duration interval explicitly classified as `moderate` or `vigorous`. The contract mirrors the bounded source semantics available from Health Connect's feature-gated activity-intensity record without enabling a Health Connect dependency, permission request, duration aggregation, or weighted intensity-minute calculation.
- `exercise.session` remains valid without classification detail and now optionally preserves a namespaced source classification `system` + opaque `code` with an optional source label. This records the source's own exercise semantics without inventing or claiming a GoreeCloud cross-provider exercise taxonomy.
- `sleep.session` remains valid without stage detail and optionally accepts source-supported stage intervals using the bounded vocabulary `unknown`, `awake`, `sleeping`, `out-of-bed`, `awake-in-bed`, `light`, `deep`, and `rem`. Stage intervals share the parent session's source/provenance and must remain ordered, non-overlapping, and within the session.
