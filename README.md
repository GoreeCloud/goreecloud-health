...
- `sleep.session` remains valid without stage detail and now optionally accepts source-supported stage intervals using the bounded vocabulary `unknown`, `awake`, `sleeping`, `out-of-bed`, `awake-in-bed`, `light`, `deep`, and `rem`. Stage intervals share the parent session's source/provenance and must remain ordered, non-overlapping, and within the session.
