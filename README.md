...
- Versioned source-level `goreecloud.health.record.v1` envelope and source contract plus nine current type contracts: `activity.steps`, `activity.distance`, `activity.active-energy`, `activity.intensity`, `exercise.session`, `sleep.session`, `heart.rate`, `body.weight`, and `hydration.water`; all current fixtures are synthetic and fail-closed validation is repository-owned.
