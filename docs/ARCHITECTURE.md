...
- `health-reconciliation-policy.v1.json` defines the fail-closed `goreecloud.health.reconciliation-policy.v1` baseline for exactly those seven current record types.
...
This boundary prevents future clients from silently manufacturing multi-source totals or choosing a winning source before domain-specific rules are governed. Positive aggregation, source-priority, conflict-resolution, or derived-value semantics remain separate work.
