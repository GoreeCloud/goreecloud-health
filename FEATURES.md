...
- `exercise.session` preserves a positive-duration session and may optionally retain the source's own exercise classification as a bounded namespaced `system`, opaque `code`, and optional display `label`. Unclassified sessions remain valid. No GoreeCloud canonical exercise category or cross-provider equivalence is inferred.
...
- Synthetic-only fixtures for all ten currently supported source-contract types, including both unclassified and source-classified exercise examples plus stage-less and staged sleep examples; none are user health data.
- Fail-closed contract validation covering unknown fields, source/provenance boundaries, record identifiers, interval ordering/session duration, type-specific units, bounded measurement values, source-classification namespace/code/label constraints, active-time consistency, activity-intensity classification/duration, and sleep-stage containment/order/overlap/type/timestamp boundaries.
- Machine-readable `goreecloud.health.reconciliation-policy.v1` in `contracts/health-reconciliation-policy.v1.json` covering exactly the ten current record types. Exercise classification and sleep stages remain nested session details rather than separate record types.
...
- A governed GoreeCloud cross-provider exercise taxonomy or mapping/equivalence rules across provider-specific activity codes.
...
- Additional exercise details such as laps, segments, routes, planned-session linkage, perceived exertion, titles, or notes.
